...
@Component({
  selector: 'app-dialog-khutbah',
  templateUrl: './dialog-khutbah.component.html',
  styleUrls: ['./dialog-khutbah.component.scss']
})
export class DialogKhutbahComponent implements OnInit {
  ...
  constructor(
    ...
    this.khutbahData = sourceData.data;
    if(this.khutbahData == null) {
      this.khutbahData = {};
      this.isCreated = true;
      this.khutbahData.id = new Date().getTime().toString() + '' + [Math.floor((Math.random() * 1000))];
    } else {
        ...
        return new Promise(async (resolve, reject) => {
          if (file.type === 'image/jpeg' || file.type === 'image/png' || file.type === 'image/jpg') { // File types supported for image
            ...
            // Sample API Call
              ...
              //   alert('err '+ err);
              // });
              return this.api.postCrImg('khutbah/quilluploadfoto/'+this.khutbahData.id, input)
                .then(result => {
  ...
  save() {
    if(this.isCreated == true) {
      this.khutbahData.created_by = this.userData.id;
      this.api.post('khutbah', this.khutbahData).then(res => {
        if(res) {
