...
export class DialogCabangComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogCabangComponent>,
    @Inject(MAT_DIALOG_DATA) public sourceData: any,
    private api: ApiService
  ) { 
    Loading.pulse();
    this.serverImg = this.common.photoBaseUrl+'cr/';
    this.dataCabang = sourceData.data;
    if(this.dataCabang == null) {
      this.dataCabang = {};
      this.isCreated = true;
      this.dataCabang.id = new Date().getTime().toString() + '' + [Math.floor((Math.random() * 1000))];
    } else {
  ...
  generateMap(data:any)
  {
    if(this.dataCabang.pin != null) {
      data = JSON.parse(this.dataCabang.pin);
    }
    var features = [];
    if(data == undefined) {
    ...
    this.map.on('singleclick', (evt) => {
      ...
      this.dataCabang.pin = JSON.stringify(dt);
        ...
        return new Promise(async (resolve, reject) => {
          if (file.type === 'image/jpeg' || file.type === 'image/png' || file.type === 'image/jpg') { // File types supported for image
            ...
            // Sample API Call
              ...
              //   alert('err '+ err);
              // });
              return this.api.postCrImg('sicara/quilluploadfoto/'+this.dataCabang.id, input)
                .then(result => {
  ...
  save() {
    this.api.put('sicara/sicara_pcm/'+this.dataCabang.id, this.dataCabang).then(res => {
      if(res) {
